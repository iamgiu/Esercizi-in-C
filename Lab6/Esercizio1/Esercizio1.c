...
void stampa(log corsa[N], int tot);
...
void leggiFile(log corsa[N], int *tot) {
  ...
  *tot = totale;
  ...
  for(int i = 0; i < totale; i++) {
     fscanf(fp, " %6s %30s %30s %30s %30s %30s %d", corsa[i].codice,corsa[i].partenza, corsa[i].destinazione, corsa[i].data, corsa[i].oraP, corsa[i].oraA, &corsa[i].ri);
       }
...
void stampa(log corsa[N], int tot) {
  ...
  for (int i = 0; i < tot; i++) {
    printf("Corsa %d:\n", cont);
    printf("Codice: %6s\nPartenza: %30s\nDestinazione: %30s\nData: %30s\nOra Partenza: %30s\nOra Arrivo: %30s\nRitardo: %d\n\n", corsa[i].codice,corsa[i].partenza, corsa[i].destinazione, corsa[i].data, corsa[i].oraP, corsa[i].oraA, corsa[i].ri);
    cont++;
  }
  return;
}
